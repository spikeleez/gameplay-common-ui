...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GameplaySettingDetailView)

#define LOCTEXT_NAMESPACE "GameplaySetting"

UGameplaySettingDetailView::UGameplaySettingDetailView(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
	, ExtensionWidgetPool(*this)
{
}

void UGameplaySettingDetailView::NativeOnInitialized()
{
...
void UGameplaySettingDetailView::NativeConstruct()
{
	Super::NativeConstruct();
}

void UGameplaySettingDetailView::ReleaseSlateResources(bool bReleaseChildren)
{
...
void UGameplaySettingDetailView::FillSettingDetails(UGameplaySetting* InSetting)
{
	...
	if (Box_DetailsExtension)
	{
		...
		if (InSetting)
		{
			TArray<TSoftClassPtr<UGameplaySettingDetailExtension>> ExtensionClassPtrs;
			if (VisualData)
			{
			...
			for (TSoftClassPtr<UGameplaySettingDetailExtension> SoftClassPtr : ExtensionClassPtrs)
			{
			...
			{
				TWeakObjectPtr<UGameplaySetting> SettingPtr = InSetting;
				StreamingHandle = UAssetManager::GetStreamableManager().RequestAsyncLoad(MoveTemp(ExtensionPaths),
...
void UGameplaySettingDetailView::CreateDetailsExtension(UGameplaySetting* InSetting, TSubclassOf<UGameplaySettingDetailExtension> ExtensionClass)
{
	if (InSetting && ExtensionClass)
	{
		if (UGameplaySettingDetailExtension* Extension = ExtensionWidgetPool.GetOrCreateInstance(ExtensionClass))
		{
